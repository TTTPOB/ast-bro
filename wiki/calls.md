...
| R          | `call` with `identifier`, `namespace_operator`, or `extract_operator` function | none | Functions are assignment expressions; `pkg::fun()` and `obj$method()` retain their receiver on the raw edge |
